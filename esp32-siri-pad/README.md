...
公开版本的中文字体为 Noto Sans SC 派生的 **SiriPadCJK** 20 px 字形子集，许可为 SIL OFL 1.1，见 `licenses/SiriPadCJK-OFL.txt`。生成后的 C 文件已包含，可直接构建；不发布个人系统字体、现场录音、Flash/NVS 备份或工具缓存。当前板子未因上传代码重新刷写，因此公开版本字形与现场上一版略有不同。
