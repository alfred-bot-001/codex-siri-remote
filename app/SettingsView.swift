...
struct SettingsView: View {
    ...
    private enum Tab: String, CaseIterable { case tuning = "参数调节", layout = "按键配置" }
    ...
    private var deviceSection: some View {
        Section {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ForEach(device.interfaces) { i in
                                HStack(spacing: 8) {
                                    ...
                                    Spacer()
                                    Text("输入 \(i.maxInput) · 特征 \(i.maxFeature)")
                                        .font(.system(size: 10, design: .monospaced))
            ...
            Text(device.updatedAt == nil
                 ? "当前版本未启用遥控器麦克风；语音输入使用输入法所选的麦克风。"
                 : "电量和固件信息来自系统蓝牙服务。当前版本未启用遥控器麦克风。")
                .font(.system(size: 11))
        }
    }
    ...
    private var cursorSection: some View {
        Section {
            ...
            Toggle(isOn: $model.tune.findCursorEnabled) {
                rowLabel("晃动遥控器寻找光标", "cursorarrow.rays")
            }
            Toggle(isOn: $model.tune.focusFollowsCursor) {
                rowLabel("遥控器移动后自动聚焦", "macwindow.on.rectangle")
            }
        } header: {
            Text("光标")
        } footer: {
            Text("提高防抖程度可忽略手指抖动，便于稳定点击。快速来回晃动遥控器时，光标周围会闪现圆环，方便定位。\n\n开启后，仅在遥控器移动光标并停稳时，聚焦光标所在且可见面积达到屏幕 90% 的应用。启动、普通鼠标移动、拖拽以及手动切换应用不会触发。")
        }
    }

    private var accelerationSection: some View {
        Section {
            slider(icon: "tortoise.fill", title: "慢速移动倍率",
                   value: $model.tune.accelMin, range: 0.2...1.0,
                   minIcon: "tortoise.fill", maxIcon: "cursorarrow.motionlines",
                   display: { String(format: "%.2f×", $0) })
            slider(icon: "hare.fill", title: "快速移动倍率",
                   value: $model.tune.accelMax, range: 1.0...4.0,
                   minIcon: "cursorarrow.motionlines", maxIcon: "hare.fill",
                   display: { String(format: "%.2f×", $0) })
            slider(icon: "arrow.down.forward", title: "低速阈值",
                   value: $model.tune.accelLowSpeed, range: 0.002...0.03,
                   minIcon: "tortoise.fill", maxIcon: "hare.fill",
                   display: { String(format: "%.0f", $0 * 1000) })
            slider(icon: "arrow.up.forward", title: "高速阈值",
            ...
            Text("在移动速度基础上，缓慢滑动时减小光标位移，便于精确定位；快速滑动时增大位移，便于跨屏移动。低于低速阈值使用慢速倍率，高于高速阈值使用快速倍率，中间平滑过渡。")
        }
    }
    ...
    private var clickSection: some View {
        Section {
            slider(icon: "hand.tap.fill", title: "按压灵敏度",
                   ...
                   display: { String(format: "%.2f", $0) })
            slider(icon: "arrow.up.and.down.and.arrow.left.and.right", title: "移动容差",
            ...
            Text("按下点击时会暂时固定光标，防止漂移。灵敏度数值越低，越容易固定；移动容差越高，越不容易感觉卡住。")
        }
    }
    ...
    private var buttonsSection: some View {
        Section {
            ...
            slider(icon: "hand.tap.fill", title: "双击间隔",
            ...
            Text("长按时间：进入第一阶段长按所需的时长，松开时执行选中的操作。双击间隔：两次点击之间允许的最长时间。桌面切换超时：长按方向环上键进入桌面切换后，超过此时间未操作左右键则退出。")
        }
    }
    ...
    private var circularSection: some View {
        Section {
            Toggle(isOn: $model.tune.circularEnabled) {
                rowLabel("圆周滚动", "arrow.clockwise")
            }
            if model.tune.circularEnabled {
                slider(icon: "circle.dashed", title: "外圈范围",
    ...
    private var startupSection: some View {
        Section {
            Toggle(isOn: Binding(
                ...
                rowLabel("登录时自动启动", "arrow.up.forward.app")
            }
            .disabled(LaunchAtLogin.state == .unavailable)
        } header: {
            Text("启动")
        } footer: {
            Text(launchAtLoginError.map { "修改失败：\($0)" }
                 ?? LaunchAtLogin.note
                 ?? "登录后自动运行 HyperVibe，也可在「系统设置 → 通用 → 登录项」中管理。")
                .font(.system(size: 11))
    ...
    private var footerSection: some View {
        Section {
            Button(role: .destructive) {
                ...
                rowLabel("恢复默认参数", "arrow.counterclockwise")
            }
        } footer: {
            Text("按键和手势配置保存在 ~/.config/siriremote/config.jsonc")
                .font(.system(size: 11))
        }
    }
